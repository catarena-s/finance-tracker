...
- [ ] 14. Реализация истории транзакций из шаблона
  - [ ] 14.1 Backend: Добавить эндпоинт для получения транзакций по шаблону
    - Создать `GET /api/v1/recurring-transactions/{id}/transactions` с пагинацией
    - Добавить метод `RecurringTransactionService.get_generated_transactions()`
    - Добавить метод `TransactionRepository.get_by_template_id()`
    - Транзакции сортируются по дате (новые первыми)
    - Возвращать 404 для несуществующего шаблона
    - _Требования: 8.8_
  
  - [ ] 14.2 Backend: Написать integration тесты
    - Тестировать успешное получение истории транзакций
    - Тестировать пустую историю (шаблон без транзакций)
    - Тестировать 404 для несуществующего шаблона
    - Тестировать пагинацию
    - Тестировать сортировку (новые первыми)
    - _Требования: 8.8_
  
  - [ ] 14.3 Frontend: Создать компонент RecurringTransactionHistory
    - Создать компонент для отображения истории транзакций
    - Загрузка транзакций через API
    - Отображение списка с пагинацией (20 на страницу)
    - Показ пустого состояния: "Транзакции ещё не создавались"
    - Индикатор загрузки и обработка ошибок
    - _Требования: 8.8_
  
  - [ ] 14.4 Frontend: Интегрировать в RecurringTransactionCard
    - Добавить кнопку "История" в карточку шаблона
    - Открывать модальное окно с компонентом RecurringTransactionHistory
    - Добавить метод `getRecurringTransactions()` в recurringTransactionService
    - _Требования: 8.8_
  
  - [ ] 14.5 Frontend: Написать unit тесты
    - Тестировать рендеринг списка транзакций
    - Тестировать пустое состояние
    - Тестировать пагинацию
    - Тестировать индикатор загрузки
