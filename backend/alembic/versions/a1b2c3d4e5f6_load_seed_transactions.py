...
def upgrade() -> None:
    ...
    for _, category_name, _ in INCOME_CATEGORIES:
        result = conn.execute(
            sa.text(
                f"SELECT id FROM categories WHERE name = '{category_name}' AND type = 'income'"
            )
        )
        row = result.fetchone()
        if row:
    ...
    for _, category_name, _ in EXPENSE_CATEGORIES:
        result = conn.execute(
            sa.text(
                f"SELECT id FROM categories WHERE name = '{category_name}' AND type = 'expense'"
            )
        )
        row = result.fetchone()
        if row:
    ...
    for i in range(0, len(transactions), 50):
        ...
        for amount, category_id, description, date, trans_type in batch:
            ...
            values.append(
                f"(gen_random_uuid(), {amount}, 'RUB', '{category_id}', '{description}', '{date}', '{trans_type}', false, NOW(), NOW())"
            )
