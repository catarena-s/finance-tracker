...
### Ключевые изменения (Настройки приложения, Административные API, Динамическое расписание)

- **Backend:** добавлена таблица `app_settings` для хранения настроек приложения (время запуска задач).
- **API:** новые эндпоинты для управления настройками (`GET/PUT /api/v1/settings`).
- **API:** административный эндпоинт для принудительного запуска создания повторяющихся транзакций (`POST /api/v1/admin/tasks/run-recurring`).
- **Celery Beat:** динамическое чтение времени запуска задачи из БД при старте (настройки `recurring_task_hour` и `recurring_task_minute`).
- **Тесты:** добавлены тесты для API настроек и административных задач; все 79 backend тестов проходят.
- **Исправления:** устранено дублирование повторяющихся транзакций при установке галочки на существующую транзакцию (next_occurrence теперь вычисляется как следующая дата после даты транзакции).

## 0.0.6 - 2026-02-22

### Ключевые изменения (Dashboard, Валюты, Повторяющиеся транзакции)
...
- **Повторяющиеся транзакции:** автоматическое создание шаблона при установке `is_recurring=true`; исправлено создание через UI.
- **Тесты:** исправлена проблема удаления production таблиц (тесты используют `finance_tracker_test`); исправлены TypeScript ошибки; обновлены тесты для новой реализации dashboard.
